...
admin.html         슈퍼 어드민 로그인 + 검수 대시보드(승인·반려, 본문 열람)
...
`/admin.html` 에서 암호를 입력해 로그인합니다(20자 미만이면 버튼 비활성화). 인증 후
검수 대기/공개/반려/전체 탭으로 항목을 보고, 본문(복호화)을 확인한 뒤 **승인·공개**
또는 **반려**할 수 있습니다. 암호는 `sessionStorage`에만 보관되어 탭을 닫으면 사라집니다.
index.html 푸터의 "담당자 로그인" 링크로도 접근할 수 있습니다.
